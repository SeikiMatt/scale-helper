...
export function generateScale({ root, mode }: GenerateScale): number[] | null {
...
export function pitchToNote({ scale, degree }: PitchToNote) {
  const noteSequences: Indexable = {
    flat: ["C", "D♭", "D", "E♭", "F♭", "F", "G♭", "G", "A♭", "A", "B♭", "C♭"],
    natural: ["C", "C♯", "D", "D♯", "E", "F", "F♯", "G", "G♯", "A", "A♯", "B"],
    sharp: ["B♯", "C♯", "D", "D♯", "E", "E♯", "F♯", "G", "G♯", "A", "A♯", "B"],
  };

  return scale.map((_, k: number) => noteSequences[degree][scale[k]]);
}
